...
struct ubus_client *ubusd_proto_new_client(int fd, uloop_fd_handler cb)
{
    ...
    //                which must be returned by malloc(), calloc() or realloc().
    ...
    //                The contents from start to min(oldsize, newsize) will be unchanged.
    //                If newsize is larger than oldsize, new memory will not be initialized.
    //                If ptr is NULL, realloc() is same as malloc().
    //                If size == 0 && and ptr != NULL, realloc() equals to free().
    //                New pointer returned by realloc() may be different from ptr.
    //     reallocarray() - Equals to realloc(ptr, nmemb * size);
    //                Return NULL, set errno to ENOMEM and leave old memory block unchanged
    //                in the case where (nmemb * size) would overflow.
    ...
    // allocate an random id for new client
    // arg client stores an avl tree to lookup, insert and remove an client id.
    // arg id keeps an avl node and the key of this node
...
{
    // initialize an AVL tree to maintenance client id which is generated by /dev/urandom
